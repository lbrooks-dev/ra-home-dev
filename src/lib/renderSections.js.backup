...
function renderOne(s) {
  ...
  if (s.type === "ctabanner") {
    ...
    const buttons = links.map((l, i) => ({ href: l.href, label: l.label, variant: i === links.length - 1 ? "orange" : "teal", external: isExt(l.href) }));
    const vClass = {
      orange: "bg-orange text-surface hover:bg-orange-600",
      teal: "bg-teal text-surface hover:bg-teal-600",
      outline: "border border-navy text-navy hover:bg-navy hover:text-surface",
    };
    return (
      `<section class="mt-section"><div class="mx-auto max-w-content px-6">` +
      `<div class="rounded-card bg-navy px-8 py-12 text-surface md:px-12">` +
      (s.eyebrow ? `<p class="text-sm font-semibold uppercase tracking-wide text-orange-200">${esc(s.eyebrow)}</p>` : "") +
      `<h2 class="mt-2 max-w-prose text-2xl text-surface md:text-3xl">${esc(s.heading)}</h2>` +
      (body ? `<p class="mt-4 max-w-prose text-navy-100">${esc(body)}</p>` : "") +
      `<div class="mt-8 flex flex-col gap-4 sm:flex-row">` +
      buttons
        .map(
          (b) =>
            `<a href="${esc(b.href)}" class="inline-flex items-center justify-center rounded-card px-6 py-3 text-sm font-semibold transition-colors ${
              vClass[b.variant] || vClass.orange
            }"${b.external ? ' rel="noopener" target="_blank"' : ""}>${esc(b.label)}</a>`
        )
        .join("") +
      `</div></div></div></section>`
    );
